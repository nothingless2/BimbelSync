...
import { deleteAcademyAction } from "@/app/superadmin/(dashboard)/actions";
...
export function DeleteAcademyModal({ isOpen, onClose, academy }: DeleteAcademyModalProps) {
  ...
  async function handleDelete() {
    if (confirmText !== academy.path_url) {
      setError(`Ketik "${academy.path_url}" untuk mengonfirmasi.`);
      return;
    }
    ...
    const res = await deleteAcademyAction(academy.id);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6">
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2 text-center">Hapus Akademi?</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-6 text-center">
            Anda akan menghapus akademi <strong className="text-slate-700 dark:text-slate-300">"{academy.name}"</strong>. (Aksi ini bersifat Soft Delete).
          </p>

          <div className="mb-6 space-y-2">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Ketik <b>{academy.path_url}</b> untuk melanjutkan</label>
            <input 
              type="text" 
              value={confirmText}
              onChange={(e) => setConfirmText(e.target.value)}
              placeholder={academy.path_url}
          ...
          <div className="flex gap-3">
            ...
            <button 
              ...
              disabled={loading || confirmText !== academy.path_url}
            ...
            >
              {loading ? <><Loader2 size={18} className="mr-2 animate-spin"/> Menghapus...</> : "Ya, Hapus Akademi"}
            </button>
          </div>
        </div>
