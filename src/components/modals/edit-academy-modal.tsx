...
export function EditAcademyModal({ isOpen, onClose, academy, plans }: EditAcademyModalProps) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    const res = await updateAcademyAction(formData);
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-800/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Edit Akademi</h2>
              <p className="text-xs text-slate-500">Perbarui info langganan bimbel.</p>
            </div>
          </div>
