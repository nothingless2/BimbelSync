...
import { createRoomAction } from "@/app/[tenantSlug]/dashboard/master-data/rooms/actions";

export function AddRoomModal({ tenantSlug }: { tenantSlug: string }) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-800">
              <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Tambah Ruangan Baru</h2>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex gap-3 justify-end">
                ...
                >
                  {isLoading ? "Menyimpan..." : "Simpan Ruangan"}
                </button>
              </div>
            </form>
          </div>
